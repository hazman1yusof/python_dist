...
query = ("SELECT * FROM episode "
         "WHERE compcode = %s AND reg_date = %s ")

mycursor.execute(query,('13A', datetime.datetime.now().strftime('%Y-%m-%d')))
...
if row_count != 0:
    ...
    for row in myresult:
        ...
        mycursor.execute(query,('13A', row['mrn']))
        ...
        mycursor.execute(query,('13A', row['mrn'], row['episno']))
...
for row_ in epispay_arr:
    query = ("SELECT * FROM medicsof_debtor.debtormast "
         "WHERE compcode = %s AND debtorcode LIKE %s ")

    mycursor.execute(query,('13A', '%'+row_['payercode']))
